...
{
    public static class GetPurchaseOrdersByStatus
    {
        public record Query(PurchaseOrderStatus Status) : IRequest<IEnumerable<PurchaseOrderDto>>;
        ...
        public class Handler : IRequestHandler<Query, IEnumerable<PurchaseOrderDto>>
        {
            ...
            {
                var purchaseOrders = await _purchaseOrderRepository.GetByStatusAsync(request.Status, cancellationToken);
