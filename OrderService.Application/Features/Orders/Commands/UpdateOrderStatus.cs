...
namespace OrderService.Application.Features.Orders.Commands
{
    ...
    {
        public record Command(int OrderId, UpdateOrderStatusDto StatusDto) : IRequest<OrderDto>;
        ...
        public class Handler : IRequestHandler<Command, OrderDto>
        {
            ...
            public async Task<OrderDto> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                if (oldStatus != OrderStatus.Cancelled && request.StatusDto.Status == OrderStatus.Cancelled)
                {
                    // Release inventory reservations for cancelled orders
